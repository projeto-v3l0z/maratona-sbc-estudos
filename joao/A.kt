...
fun stdinLines(): Sequence<String> = System.`in`.bufferedReader().lineSequence()

object A {
    @JvmStatic
    fun main(args: Array<String>) {
        val lines = stdinLines().iterator()
        val firstLine = lines.next()
        val (n, m) = firstLine.split(" ").map { it.toInt() }
        val maxByTurma = IntArray(m)

        repeat(n) {
            val values = lines.next().split(" ").map { it.toInt() }

            values.forEachIndexed { i, v -> 
                if (v > maxByTurma[i]) {
                    maxByTurma[i] = v 
                    // println("UPDATE: maxByTurma[$i] = ${maxByTurma[i]}")
                }
            }
        }
        println(maxByTurma.sum())
    }
}
